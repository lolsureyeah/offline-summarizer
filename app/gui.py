...
class App(ctk.CTk):
    ...
    def _create_widgets(self):
        ...
        # --- Mode Switcher & Keyword/Question Input ---
        options_frame = ctk.CTkFrame(main_frame)
        options_frame.pack(pady=10, padx=10, fill="x")
        options_frame.grid_columnconfigure(1, weight=1)

        self.mode_switcher = ctk.CTkSegmentedButton(options_frame, values=["Summarize", "Ask a Question"], command=self.switch_mode)
        self.mode_switcher.set("Summarize")
        self.mode_switcher.grid(row=0, column=0, columnspan=2, padx=10, pady=10)

        self.keywords_label = ctk.CTkLabel(options_frame, text="Focus Keywords (optional):")
        self.keywords_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")
        ...
        self.question_label = ctk.CTkLabel(options_frame, text="Your Question:")
        self.question_entry = ctk.CTkEntry(options_frame, placeholder_text="e.g., What was the main conclusion?")

        self.length_label = ctk.CTkLabel(options_frame, text="Summary Length (%):")
        self.length_label.grid(row=2, column=0, padx=10, pady=5, sticky="w")
        self.length_entry = ctk.CTkEntry(options_frame, placeholder_text="e.g., 20", width=120)
        self.length_entry.grid(row=2, column=1, padx=10, pady=5, sticky="w")
        
        # --- Action Button ---
        self.action_button = ctk.CTkButton(main_frame, text="Generate Summary", command=self.start_task_thread, state="disabled")
    ...
    def switch_mode(self, mode):
        """Hides/shows GUI elements based on the selected mode."""
        self.action_button.configure(text=f"Generate {mode.split(' ')[-1]}")
        if mode == "Summarize":
            ...
            self.keywords_entry.grid_remove()
            ...
            self.question_entry.grid(row=1, column=1, padx=10, pady=5, sticky="ew")
    ...
    def select_file(self):
        """Opens a file dialog, reads the file content, and enables the action button."""
    ...
    def start_task_thread(self):
        """Starts the backend processing in a separate thread to keep the GUI responsive."""
    ...
    def run_task_logic(self, params):
        ...
        try:
            ...
            self.status_label.configure(text=f"{params['mode']} generated successfully!", text_color="lightgreen")
